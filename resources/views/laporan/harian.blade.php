...
<table class="table table-bordered table-sm">
    ...
    <tbody>
        ...
        <tr class="{{ $row->status == 'batal' ? 'table-danger' : '' }}">
            <td>{{ $key + 1}}</td>
            ...
            <td>
                @if($row->status == 'batal')
                    <del>{{ number_format($row->total, 0, ',', '.') }}</del>
    ...
    <tfoot>
        <tr>
            ...
            <th>
                @php
                    // Hitung hanya transaksi yang tidak batal
                    $totalValid = $penjualan->where('status', '!=', 'batal')->sum('total');
                @endphp
                {{ number_format($totalValid, 0, ',', '.') }}
            </th>
        </tr>
    </tfoot>
</table>
